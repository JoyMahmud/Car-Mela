<?php
    ...
    $query_result = $obj_blog_post->edit_blog_post_info_by_id($blog_post_id);
    $result = mysqli_fetch_assoc($query_result);

    $related_topic_in_string = $result['related_topic'];
    $related_topic_in_array  = explode(",", $related_topic_in_string);


    /*update Blog Post info*/
    if (isset($_POST['btn'])){
        $related_topic =$_POST['related_topic'];
        $commaSeparatedRelatedTopic=implode(",",$related_topic);
        $_POST['related_topic']=$commaSeparatedRelatedTopic;

        $obj_blog_post = new Blogpost();
        $obj_blog_post->update_blog_post_info($_POST);
    }
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="manage_blog_post.php">Manage Blog Post</a></li>
                    ...
                    <div class="well">
                        ...
                        <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                                <label for="post_title" class="col-sm-2 control-label">Blog Post Title</label>
                                <div class="col-sm-8">
                                    <input type="text" name="post_title" class="form-control" value="<?php echo $result['post_title']?>" required>
                                    <input type="hidden" name="blog_post_id" class="form-control" value="<?php echo $result['blog_post_id']?>">
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="post_details" class="col-sm-2 control-label">Blog Post Details</label>
                                <div class="col-sm-8">
                                    <textarea name="post_details" class="form-control" rows="5" required><?php echo $result['post_details']?></textarea>
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="related_topic[]" class="col-sm-2 control-label">Post Related Topic</label>
                                <div class="col-sm-1">
                                    <input type="checkbox" name="related_topic[]" value="Travel" <?php if(in_array("Travel",$related_topic_in_array)){echo "checked";}?>> Travel
                                </div>
                                <div class="col-sm-1">
                                    <input type="checkbox" name="related_topic[]" value="Journey" <?php if(in_array("Journey",$related_topic_in_array)){echo "checked";}?>> Journey
                                </div>
                                <div class="col-sm-1">
                                    <input type="checkbox" name="related_topic[]" value="Raceing" <?php if(in_array("Raceing",$related_topic_in_array)){echo "checked";}?>> Raceing
                                </div>
                                <div class="col-sm-2">
                                    <input type="checkbox" name="related_topic[]" value="Technology" <?php if(in_array("Technology",$related_topic_in_array)){echo "checked";}?>> Technology
                                </div>
                                <div class="col-sm-">
                                    <input type="checkbox" name="related_topic[]" value="Invention" <?php if(in_array("Invention",$related_topic_in_array)){echo "checked";}?>> Invention
                                </div>
                            </div>


                            <div class="form-group">
                                <label for="post_Date" class="col-sm-2 control-label">Blog Post Date</label>
                                <div class="col-sm-8">
                                    <input type="date" name="post_Date" placeholder="8 May 2015" class="form-control" value="<?php echo $result['post_Date']?>" required>
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="post_image" class="col-sm-2 control-label">Blog Post Image</label>
                                <div class="col-sm-8">
                                    <input type="file" name="post_image" accept="image/*"><br>
                                    <img src="../../../<?php $len = strlen($result['post_image']); $image = substr($result['post_image'],9,$len); echo $image;?>" alt="post_image" height="100" width="100">
                                </div>
                            </div>
                            <div class="form-group">
